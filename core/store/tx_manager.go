...
func (txm *EthTxManager) SignedRawTxWithBumpedGas(originalTx models.Tx, gasLimit uint64, gasPrice big.Int) (slice []byte, err error) {
	ma := txm.getAccount(originalTx.From)
	rlp := new(bytes.Buffer)
	if ma == nil {
		return slice, fmt.Errorf("Unable to locate %v as an available account in EthTxManager. Has TxManager been started or has the address been removed?", originalTx.From.Hex())
	}
